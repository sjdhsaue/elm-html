<template>
  <div class="wrapper">
    ...
    <!-- 推荐商家列表部分 -->
    <ul class="business" >
      <li v-for="item in this.data" :key="item.id" @click="toBusinessInfo(item.id)" >
...
export default {
    ...
    methods :{
      fetchData () {
        axios.get('http://localhost:8082/business-center/business/list')
            .then(response => {
      ...
      toBusinessInfo(id){
        localStorage.setItem('business', JSON.stringify(id))
